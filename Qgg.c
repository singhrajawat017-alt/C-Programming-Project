...
void saveBookingToFile(Booking b) {
    ...
    fprintf(fp, "Name: %s | Phone: %s | Movie: %s | Row: %d | Seat: %d | Price: ₹%d\n",
...
void bookTicket(int m) {
    ...
    printf("Name: %s\nPhone: %s\nMovie: %s\nRow: %d Seat: %d\nPrice: ₹%d\n",
           b.customerName, b.phone, b.movieName, b.row, b.col, b.price);
    printf("\n💳 Pay here: https://payment.example.com/pay/%s%d%d\n",
...
void cancelTicket(int m) {
    ...
    int price = getPrice(row);
    movies[m].revenue -= price;
    printf("Ticket canceled successfully! Refunded: ₹%d\n", price);
}
